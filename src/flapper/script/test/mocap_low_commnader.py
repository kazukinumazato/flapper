...
def tune_pid_gains(cf):
    cf.param.set_value('posCtlPid.xKp', 0.75)
    cf.param.set_value('posCtlPid.yKp', 0.75)
    cf.param.set_value('pid_attitude.roll_kp', 20.0)
    cf.param.set_value('pid_attitude.pitch_kp', 26.0)

def print_params(cf):
    print(cf.param.get_value('posCtlPid.xKp'))
...
def run_sequence(cf, trajectory_id, duration):
    ...
    low_level_commander = cf.commander
    try:
        print_params(cf)
        ...
        # time.sleep(duration)
        time.sleep(3)
        low_level_commander.send_velocity_world_setpoint(1, 0, 0, 0)
        time.sleep(30)
        commander.land(0.27, 2.0)
        time.sleep(5.0)
        commander.stop()
