    ...
    def execute(self, userdata):
        if self.robot.state != self.start_flight_state:
            rospy.logwarn(
                ...
                + ": the robot state ({}) is not at the start_flight_state ({}).".format(
                    self.robot.state, self.start_flight_state
                )
            )
            return "failure"
        self.func()
        ...
        rospy.logwarn(
            self.__class__.__name__
            + ": the robot state ({}) is not at the goal_flight_state ({}).".format(
                self.robot.state, self.goal_flight_state
            )
        )
        return "failure"
...
class PalmLand(SingleCommandState):
    def __init__(self, robot, func):
        SingleCommandState.__init__(
            self, robot, func, RobotState.PALM_LAND_READY, RobotState.STOP
        )
