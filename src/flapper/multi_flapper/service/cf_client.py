...
class CfClient():
    def __init__(self, uri, cf_logger, drone_id):
        ...
        self.mocap_topic_name = f'mocap_node/mocap/flapper{drone_id}/pose'
    ...
    def connected_callback(self, uri):
        rospy.loginfo(f'[CF{self.drone_id}]Connected to {uri}')
        ...
        for log_config in self.cf_logger.log_configs:
            try:
                ...
                rospy.logerr(f'[CF{self.drone_id}]Could not add log config, bad configuration.')
                                                                                                
        for log_config in self.cf_logger.log_configs:
            try:
                log_config.start()
            except KeyError as e:
                rospy.logerr(f'[CF{self.drone_id}]Could not start log configuration,'
                             '{} not found in TOC'.format(str(e)))
    
    def fully_connected_callback(self, uri):
        rospy.loginfo(f'[CF{self.drone_id}]Fully connected to %s' % uri)
        self.adjust_orientation_sensitivity()
        self.activate_kalman_estimator()
        self.activate_pid_controller()
        self.tune_pid_gains()
        self.mocap_sub = rospy.Subscriber(self.mocap_topic_name,PoseStamped,self.mocap_sub_callback)
        
    def connection_failed_callback(self, uri, msg):
        rospy.logerr(f'[CF{self.drone_id}]Connection to {uri} failed: {msg}')
                                                                
    def connection_lost_callback(self, uri, msg):
        rospy.logerr(f'[CF{self.drone_id}]Connection to {uri} lost: {msg}')
        
    def disconnected_callback(self, uri):
        rospy.logerr(f'[CF{self.drone_id}]Disconnected from {uri}')
