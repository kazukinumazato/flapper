...
class Navigator():
    ...
    def __init__(self, deceleration_radius = 1.0, dist_close = 0.3):
        ...
        # 制御の開始・停止指示を受け取る
        self.approach_start_sub = rospy.Subscriber('approach_start',Empty,self.approach_start_sub_callback)
        self.approach_stop_sub = rospy.Subscriber('approach_stop',Empty,self.approach_stop_sub_callback)
    ...
    def run(self, timeout=120):
        ...
        while rospy.get_time() <= start_t + timeout and self.running:
            ...
            if distance_chest_drone_on_XY_plane < distance_chest_hand_on_XY_plane:
                ...
                vector_c_h = np.array([hand_pos.x - chest_pos.x, hand_pos.y - chest_pos.y]) # 胸→手
                vector_c_d = np.array([drone_pos.x - chest_pos.x, drone_pos.y - chest_pos.y]) # 胸→ドローン
            ...
            # 条件2: 胸からは十分離れている場合
                ...
                # 条件2-B: 手からかなり遠い場合（全速で接近圏内へ移動）
                    ...
                    vector_d_h = np.array([hand_pos.x - drone_pos.x, hand_pos.y - drone_pos.y]) # ドローン→手
                    vector_d_h_normalized = vector_d_h / np.linalg.norm(vector_d_h) # 単位ベクトル化
