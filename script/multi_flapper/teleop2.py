...
class Teleop():
    ...
    def approach_sub_callback1(self, msg):
        yaw = extract_yaw(msg.orientation)
        rospy.loginfo('[CF1] Approach to x: %f, y: %f, z: %f, yaw: %f' % (msg.position.x, msg.position.y, msg.position.z, np.rad2deg(yaw)))
        self.mm1.approach(msg.position.x, msg.position.y, msg.position.z, yaw)

    def approach_sub_callback2(self, msg):
        yaw = extract_yaw(msg.orientation)
        rospy.loginfo('[CF2] Approach to x: %f, y: %f, z: %f, yaw: %f' % (msg.position.x, msg.position.y, msg.position.z, np.rad2deg(yaw)))
