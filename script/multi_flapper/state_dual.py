...
class Takeoff(SingleCommandState):
        ...
        def execute(self, userdata):
            ...
            while not self.robot.are_both_in_state(RobotState.HOVER):
                if rospy.is_shutdown(): return 'failure'
                rospy.sleep(0.1)
...
class Approach(State):
        def __init__(self, robot, timeout=120, safety_radius = 0.30, threshold = 0.1):
                ...
                self.hand_pose_sub = rospy.Subscriber('mocap_node/mocap/hand1/pose',
                                                      PoseStamped,
                                                      self.hand_pose_sub_callback)
                self.drone_pose_sub = rospy.Subscriber('mocap_node/mocap/flapper1/pose',
                                                       PoseStamped,
        ...
        def execute(self, userdata):
                ...
                while rospy.get_time() < start_t + self.timeout:
                        if rospy.is_shutdown():
                                return 'failure'
                        # MoCapデータが揃っているかチェック
                        if not hasattr(self, 'chest_pose') or not hasattr(self, 'hand_pose') or not hasattr(self, 'drone_pose'):
                            rospy.logwarn('MoCap data not yet available for Approach transition check.')
                            rospy.sleep(0.1)
                            continue
        ...
        def execute(self, userdata):
                ...
                while rospy.get_time() < start_t + self.timeout:
                        ...
                        if not hasattr(self, 'chest_pose') or not hasattr(self, 'hand_pose'):
                                rospy.logwarn('Unable to get chest_pose/hand_pose. Check mocap settings.')
                                continue
                        chest_pos = self.chest_pose.position
                        hand_pos = self.hand_pose.position
                        distance_chest_hand = dist(chest_pos, hand_pos)
